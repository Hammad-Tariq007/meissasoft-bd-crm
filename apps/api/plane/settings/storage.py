...
class S3Storage(S3Boto3Storage):
    ...
    def __init__(self, request=None):
        ...
        # Optional browser-facing endpoint. When set, presigned URLs are signed
        # against this host while server-side operations keep using
        # aws_s3_endpoint_url. This resolves split-horizon setups where the API
        # container and the browser reach the object store on different hosts
        # (e.g. plane-minio:9000 internally vs localhost:9000 from the browser).
        ...
        if os.environ.get("USE_MINIO") == "1":
            ...
            server_endpoint_url = self.aws_s3_endpoint_url

        # Client for real server-side operations (upload, delete, copy, head).
        # Must point at an endpoint reachable from the API/worker containers.
        ...
        # Client used to sign presigned URLs handed to the browser. Signing is
        # offline (no connectivity needed), so this only needs a host the browser
        # can reach. Falls back to the server client when no public endpoint is
        # configured, preserving the previous single-endpoint behavior.
        if self.aws_s3_public_endpoint_url and self.aws_s3_public_endpoint_url != server_endpoint_url:
